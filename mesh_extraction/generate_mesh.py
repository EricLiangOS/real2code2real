...
parser.add_argument("--generation_size", default = 10, type=int)
...
for obj in range(num_objects):
    ...
    object_mask_dir = os.path.join(object_dir, "images")
    ...
    object_output_dir = os.path.join(base_output_dir, f"object_{obj + 1}")
    generate_frequency = len(os.listdir(object_mask_dir)) * 1.0/args.generation_size

    os.makedirs(object_output_dir, exist_ok=True)
    for index in range(args.generation_size):
        source_path = os.path.join(object_mask_dir, os.listdir(object_mask_dir)[int(index * generate_frequency)])
        output_path = os.path.join(object_output_dir, f"object_{obj + 1}_angle_{index}.glb")

        generate(source_path, output_path, pipeline)
