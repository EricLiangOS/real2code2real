...
def get_object_masks(object_dir, prompts):
    ...
    if "sam_masks.npz" in os.listdir(object_input_dir):
        ...
        # save_masks(video_segments, object_input_dir)
        ...
        for index in video_segments:
            ...
            # for obj in objects:
            #     mask = np.logical_or(mask, video_segments[index][obj])
...
parser = ArgumentParser("Apply SAM 2 on multiple objects")
parser.add_argument("--base_dir", "-s", required=True, type=str)
...
colmap_output_dir = os.path.join(base_dir, "colmap_output")

# Reduce the dataset size
if not os.path.isdir(input_dir):
    print(f"Making new directory at {input_dir}")
...
if not os.path.isdir(labeled_dir):
    print(f"Now getting labeled images and saving at {labeled_dir}")
...
for obj in range(num_objects):
    ...
    if not os.path.isdir(object_input_dir):
        print(f"Making copies of input in object directory for object {obj + 1}")
        ...
        print(f"Object input directory already found for object {obj + 1}")
    
    # Get masks for the moved directory
    if not os.path.isdir(os.path.join(object_dir, "images")):
        print(f"Getting masks for object {obj + 1}")
        get_object_masks(object_dir, all_object_prompts[obj])
    else:
        print(f"Masks directory already found for object {obj + 1}")
